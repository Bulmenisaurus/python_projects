    ...
    for wrd in wordpattern_data:
        for index, lttr in enumerate(wrd):
            try:  # detect if lttr is last
                ...
                wordpatterns[lttr] = ' '  # space (should) mean End of word
            else:
...
# print(word_data)
generated_words = gen_words_from_patterns(word_data, amount=20, min_len=3, capitalize=True)
print(sorted(generated_words))
